...
export const AuthProvider = ({ children }) => {
    ...
    try {
      ...
      if (!error.response) {
        ...
        switch (status) {
          ...
          case 400:
            message =
              serverMessage || "Please provide valid email and password.";
    ...
    try {
      ...
      if (!error.response) {
        ...
        switch (status) {
          case 400:
            message =
              serverMessage || "User already exists or invalid data provided.";
            break;
          case 429:
  ...
  useEffect(() => {
    ...
    console.log(
      "[Auth] Starting automatic token refresh interval (every 6 minutes)",
    );
    ...
    const refreshInterval = setInterval(
      async () => {
        ...
        try {
          ...
          console.warn(
            "[Auth] Refresh attempt 1 failed, retrying in 1 second...",
          );
          toast.info("Connection issue. Retrying session refresh...", {
            duration: 2000,
          });
          ...
          try {
            ...
            if (isNetworkError || isServerError) {
              ...
              console.warn(
                "[Auth] Refresh failed due to network/server issue. User can continue.",
                {
                  isNetworkError,
              ...
              const message =
                secondError.response?.data?.error?.message || "Session expired";
